...
function Toast({ message, type = 'error', onClose }) {
  ...
  return (
    <div className={`fixed top-20 right-6 z-[200] px-4 py-2.5 flex items-center gap-3 rounded-xl text-white text-sm font-medium bg-gradient-to-r ${colors[type]} border backdrop-blur-xl shadow-2xl animate-[slideIn_0.3s_ease]`}>
      <span>
...
function parseMarkdown(text) {
  ...
  // Inline code
  result = result.replace(/`([^`]+)`/g, '<code class="px-1.5 py-0.5 rounded bg-white/10 text-[#6ee7b7] text-[13px] font-mono">$1</code>')
...
function parseMarkdownTables(text) {
  ...
  while (i < lines.length) {
    ...
    if (i + 1 < lines.length &&
      ...
      tableHtml += '<thead><tr>'
      headers.forEach(h => {
        tableHtml += `<th class="px-4 py-2.5 text-left text-[#6ee7b7] font-semibold bg-white/[0.04] border-b border-white/[0.08]">${h}</th>`
      })
...
function ProductCard({ product, onClick, onSimulatePayment }) {
  ...
  return (
    <div className="my-0.5 rounded-2xl overflow-hidden bg-white/[0.03] border border-white/[0.08] shadow-md max-w-[340px] transition-all duration-300 group flex flex-col hover:-translate-y-1 hover:shadow-[0_8px_20px_rgba(108,99,255,0.15)] hover:border-[#10b981]/30 relative">
      <div className="absolute inset-0 bg-gradient-to-t from-[#10b981]/10 to-transparent opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-300" />
      <div onClick={() => onClick?.(product)} className="flex px-3 pt-3 pb-2 gap-3 cursor-pointer hover:bg-white/[0.05] transition-colors relative z-10">
        ...
        <div className="flex flex-col justify-center min-w-0 flex-1 py-1">
          <h4 className="font-semibold text-[14px] text-white leading-snug group-hover:text-[#6ee7b7] transition-colors truncate">{product.name}</h4>
        ...
        {/* Arrow */}
        <div className="flex items-center text-gray-600 group-hover:text-[#10b981] transition-colors">
        ...
        <div className="px-3 pb-3 pt-1">
          ...
          {product.orderId && onSimulatePayment && (
            <button onClick={() => onSimulatePayment(product.orderId)}
              className="flex items-center justify-center gap-1.5 w-full mt-1.5 text-[10px] font-medium text-gray-500/70 hover:text-gray-300 transition-colors cursor-pointer">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div onClick={(e) => e.stopPropagation()}
        className="relative w-full max-w-2xl max-h-[90vh] bg-[#18181b] border border-white/[0.08] rounded-3xl shadow-2xl shadow-black/50 overflow-y-auto animate-[fadeIn_0.25s_ease]" style={{ scrollbarWidth: 'thin', scrollbarColor: '#2a2a4a #18181b' }}>
        ...
        {/* Hero image Carousel */}
        {images.length > 0 && (
          <div className="w-full relative -mt-9 flex flex-col group/hero min-h-[300px] bg-[#09090b]/50">
            {imgError ? (
            ...
            {/* Gradient Overlay */}
            <div className="absolute inset-0 z-20 bg-gradient-to-t from-[#18181b] via-transparent to-transparent pointer-events-none" />
        ...
        <div className="px-7 pb-7 pt-1">
          ...
          <div className="flex items-start justify-between gap-4 mb-3">
            <div>
              ...
              {product.category && (
                <span className="inline-block mt-2 px-3 py-1 rounded-lg bg-[#10b981]/10 text-[#6ee7b7] text-[11px] font-semibold uppercase tracking-wider">
                  {product.category}
                </span>
              )}
            </div>
            ...
            {product.paymentLink ? (
              ...
              <>
                <button onClick={() => { onBuy(product); onClose() }}
                  className="flex-1 flex items-center justify-center gap-2 py-3.5 rounded-xl text-[15px] font-semibold text-white bg-gradient-to-r from-[#10b981] to-[#059669] shadow-lg shadow-[#10b981]/25 hover:-translate-y-0.5 hover:shadow-[#10b981]/40 transition-all cursor-pointer min-w-[120px]">
                  🛒 Buy Now
                </button>
                <button onClick={() => { onAddToCart(product); onClose() }}
                  className="flex-1 flex items-center justify-center gap-2 py-3.5 rounded-xl text-[15px] font-semibold text-[#6ee7b7] bg-[#10b981]/10 border border-[#10b981]/30 hover:-translate-y-0.5 hover:bg-[#10b981]/20 transition-all cursor-pointer min-w-[120px]">
                  + Add to Cart
                </button>
              </>
            ) : (
...
function useVoiceInput(onResult) {
  ...
  const startListening = useCallback(() => {
    ...
    recognition.onerror = (e) => {
      setListening(false)
      if (e.error !== 'aborted') {
        const msg = e.error === 'network' ? 'Voice recognition unavailable (Browser network error). Please type instead.' : `Voice error: ${e.error}. Please type instead.`
        onResult(null, msg)
      }
    }
...
function CartPanel({ cart, onClose, onCheckout, onRemove }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="absolute inset-0 bg-black/70 backdrop-blur-sm animate-[fadeIn_0.2s_ease]" />
      <div onClick={(e) => e.stopPropagation()}
        className="relative w-full max-w-md max-h-[80vh] bg-[#18181b] border border-white/[0.08] rounded-3xl shadow-2xl overflow-y-auto animate-[fadeIn_0.25s_ease]" style={{ scrollbarWidth: 'thin', scrollbarColor: '#2a2a4a #18181b' }}>
...
export default function App() {
  ...
  const [orderData, setOrderData] = useState({ count: 0 })
  ...
  // Fetch orders state from backend
  const fetchOrders = useCallback(async () => {
    if (!user) return
    try {
      const r = await fetch(`${API}/api/orders`, { headers: getAuthHeaders() })
      const d = await r.json()
      setOrderData({ count: d.activeCount || 0 })
    } catch { /* silent */ }
  }, [getAuthHeaders, user])

  // Sync state on message changes or login
  useEffect(() => { fetchCart(); fetchOrders() }, [msgs, user, fetchCart, fetchOrders])
  ...
  const NAV = [
    ...
    {
      label: 'Orders', active: false, badge: orderData.count > 0 ? String(orderData.count) : null, icon: <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" /></svg>, action: () => {
        if (!user) {
          setMsgs(p => [...p, { from: 'bot', text: 'Please **sign in with Google** first to view your orders. 🔐' }])
        } else {
          send('What is the status of my orders?')
        }
      }
    },
  ]
  ...
  return (
    <div className="fixed inset-0 flex bg-[#09090b] text-white font-['Inter',system-ui,sans-serif] overflow-hidden">

      {/* Mobile hamburger */}
      <button onClick={() => setSidebarOpen(!sidebarOpen)}
        className="md:hidden fixed top-5 left-5 z-50 w-10 h-10 rounded-xl bg-[#18181b] border border-white/[0.1] flex items-center justify-center text-gray-400 hover:text-white transition-colors">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 12h18M3 6h18M3 18h18" /></svg>
      </button>

      {/* ═══ SIDEBAR ═══ */}
      <aside className={`${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 fixed md:static z-40 w-72 shrink-0 h-full bg-[#09090b] border-r border-white/[0.07] flex flex-col p-6 gap-7 transition-transform duration-300`}>

        {/* Brand */}
        <div className="flex items-center gap-3">
          <div className="w-11 h-11 rounded-[14px] bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center text-xl font-extrabold shadow-lg shadow-emerald-500/15 font-['Outfit',system-ui,sans-serif]">
            Q
          </div>
          <div>
        ...
        <div className="flex flex-col gap-1">
          ...
          {NAV.map((n, i) => (
            <div key={i} onClick={() => { if (n.label === 'Cart') { setShowCart(true); fetchCart() } else if (n.action) { n.action() }; setSidebarOpen(false) }}
              className={`group flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium cursor-pointer transition-all duration-200 ${n.active
                  ? 'bg-[#10b981]/15 text-[#10b981] border border-[#10b981]/30 shadow-sm shadow-[#10b981]/10'
              ...
              {n.label}
              {n.badge && (
                <span className={`ml-auto text-[11px] font-bold rounded-md px-2 py-0.5 ${n.active ? 'bg-[#10b981]/25 text-[#6ee7b7]' : 'bg-white/[0.06] text-gray-500 group-hover:bg-white/[0.1] group-hover:text-gray-300'
                  }`}>
                  {n.badge}
                </span>
              )}
        ...
        {/* Footer */}
        <div className="mt-auto p-4 rounded-2xl bg-gradient-to-br from-[#27272a] to-[#18181b] border border-white/[0.07] backdrop-blur-sm">
      ...
      <main className="flex-1 flex flex-col h-full min-w-0 overflow-hidden">
        ...
        {/* Topbar */}
        <header className="shrink-0 flex items-center justify-between px-7 py-3 border-b border-white/[0.07] bg-[#09090b]/90 backdrop-blur-xl">
          <div className="flex flex-col pl-14 md:pl-0">
            <h2 className="text-[17px] font-semibold font-['Outfit',system-ui,sans-serif]">QuickShop</h2>
            <p className="text-xs text-gray-500 mt-0.5">Powered by Gemini &amp; Razorpay</p>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={handleClearChat} title="Clear Chat" className="flex items-center justify-center w-8 h-8 rounded-full bg-white/[0.04] border border-white/[0.06] text-gray-400 hover:text-white hover:bg-white/[0.08] transition-all">
            ...
            <div className={`${user ? 'hidden' : 'block'} hover:scale-105 transition-transform duration-200 rounded-full`}>
              <div ref={googleButtonRef} className="flex items-center justify-center"></div>
            </div>

            {user && (
              <div className="flex items-center gap-3 bg-[#18181b] border border-white/[0.05] rounded-full p-1 pr-3 shadow-md">
                <img src={user.picture || `https://ui-avatars.com/api/?name=${user.name || 'User'}&background=10b981&color=fff`} referrerPolicy="no-referrer" alt="" className="w-8 h-8 rounded-full bg-white/10" />
        ...
        {/* Messages */}
        <section className="flex-1 overflow-y-auto px-7 py-4 flex flex-col gap-5 scroll-smooth" style={{ scrollbarWidth: 'thin', scrollbarColor: '#2a2a4a #09090b' }}>
          {msgs.map((m, i) => (
            <div key={i} className={`flex flex-col ${m.from === 'user' ? 'self-end items-end max-w-[85%] md:max-w-[60%] animate-[springUp_0.4s_ease-out]' : 'self-start items-start max-w-[90%] md:max-w-[520px] animate-[springUp_0.6s_cubic-bezier(0.175,0.885,0.32,1.275)]'}`}>
              <span className={`text-[11px] font-semibold text-gray-500 mb-1.5 flex items-center gap-1.5`}>
                {m.from === 'user'
                  ? <><img src={user?.picture || `https://ui-avatars.com/api/?name=${user?.name || 'User'}&background=10b981&color=fff`} referrerPolicy="no-referrer" className="w-4 h-4 rounded-full bg-white/10" /> {user?.name?.split(' ')[0] || 'You'}</>
                  : <><span className="text-sm">🤖</span> QuickShop</>
                }
              </span>
              {m.imageUrl && (
                <img src={m.imageUrl} alt="Uploaded" className="max-w-[200px] max-h-[150px] rounded-xl mb-2 border border-white/[0.1] object-cover" />
              )}
              <div className={`px-5 py-3.5 text-[14.5px] leading-relaxed whitespace-pre-wrap break-words ${m.from === 'user'
                  ? 'bg-gradient-to-br from-[#10b981] to-[#059669] text-white rounded-2xl rounded-br-sm shadow-lg shadow-[#10b981]/20'
                  : 'bg-[#18181b] border border-white/[0.07] text-gray-200 rounded-2xl rounded-bl-sm shadow-md shadow-black/20'
                }`}>
          ...
          {loading && (
            <div className="self-start flex flex-col max-w-[75%] animate-[fadeIn_0.3s_ease]">
              ...
              <div className="px-5 py-3 bg-[#18181b] border border-white/[0.07] rounded-2xl rounded-bl-sm flex flex-col gap-2 relative overflow-hidden">
                ...
                <div className="flex items-center gap-2 relative z-10">
                  <div className="w-4 h-4 rounded-full bg-gradient-to-br from-[#10b981] to-cyan-400 animate-pulse" />
                  <span className="text-[13px] text-[#10b981] font-medium tracking-wide">Agent is thinking...</span>
                </div>
        ...
        {/* Input */}
        <footer className="shrink-0 px-4 md:px-7 pb-2 md:pb-3 pt-2 md:pt-3 bg-[#09090b]/90 backdrop-blur-xl">
          <div className="max-w-3xl mx-auto w-full flex flex-col items-center">
              ...
              <div className="flex gap-2 w-max px-4 mx-auto">
                {[...CHIPS, ...CHIPS, ...CHIPS].map((c, i) => (
                  <button key={i} onClick={() => send(c.text)} style={{ animationDelay: `${i * 60}ms` }}
                    className="shrink-0 animate-[springUp_0.5s_both] px-3.5 py-1.5 rounded-full text-[12.5px] font-medium text-gray-400 bg-[#18181b] border border-white/[0.07] hover:border-[#10b981]/40 hover:text-[#10b981] hover:bg-[#10b981]/10 hover:-translate-y-px hover:shadow-md hover:shadow-[#10b981]/10 transition-all cursor-pointer active:scale-95">
                    {c.label}
                  </button>
                ))}
              </div>
            ...
            {imagePreview && (
              <div className="flex items-center gap-2 mb-2 w-full">
                <img src={imagePreview.url} alt="Preview" className="w-14 h-14 rounded-xl object-cover border border-[#10b981]/30" />
            ...
            <div className="flex items-center gap-2 w-full bg-[#18181b] border border-white/[0.07] rounded-[18px] px-2 md:px-3 py-1 focus-within:border-[#10b981]/40 focus-within:shadow-[0_0_0_3px_rgba(16,185,129,0.12)] transition-all">
              {/* Image upload button */}
              <input ref={fileInputRef} type="file" accept="image/*" className="hidden" onChange={handleImageSelect} />
              <button onClick={() => fileInputRef.current?.click()}
                disabled={loading}
                className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0 transition-all cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed bg-transparent text-gray-500 hover:text-[#10b981] hover:bg-[#10b981]/10"
              ...
              <button onClick={listening ? stopListening : startListening}
                disabled={loading}
                className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 transition-all cursor-pointer disabled:opacity-30 disabled:cursor-not-allowed ${listening
                    ? 'bg-red-500/20 text-red-400 animate-pulse border border-red-500/30'
                    : 'bg-transparent text-gray-500 hover:text-[#10b981] hover:bg-[#10b981]/10'
                  }`}
              ...
              <button onClick={() => send()} disabled={loading || (!input.trim() && !imagePreview)}
                className="w-9 h-9 rounded-[10px] bg-[#10b981] flex items-center justify-center shrink-0 shadow-md shadow-[#10b981]/20 hover:bg-[#059669] hover:scale-105 disabled:opacity-30 disabled:cursor-not-allowed transition-all cursor-pointer active:scale-95">
      ...
      {/* Subtle ambient glow */}
      <div className="fixed -bottom-[30%] -right-[10%] w-[50vw] h-[50vw] rounded-full bg-[radial-gradient(circle,rgba(16,185,129,0.04)_0%,transparent_70%)] pointer-events-none z-0" />
